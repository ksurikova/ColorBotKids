...
extension SpeechConfiguration {
    func normalized(using caps: SpeechCapabilities) -> SpeechConfiguration {
        SpeechConfiguration(
            language: caps.locale.identifier,
            useOnlyOnDevice: caps.onDeviceAvailable ? useOnlyOnDevice : false,
            autoPlayConfirmation: caps.ttsAvailable ? autoPlayConfirmation : false
        )
    }
}
